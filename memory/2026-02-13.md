...
- **DO NOT batch-send multiple messages** — consolidate into one compiled/summarized reply
...
- Root cause: `yarn.lock` out of sync with `package.json` — Vercel runs `--immutable` by default
- Fix: ran `YARN_ENABLE_IMMUTABLE_INSTALLS=false yarn install` locally, committed updated yarn.lock, pushed
- Commit `8609627` deployed successfully — READY
...
## UI Changes
- Removed "Create" from nav → replaced with agent-only info page
- Homepage hero: removed "Create a Raffle" button, step 1 → "AI Agents Create"
...
- CREATE → BUY (3 tickets @ 1 cUSD) → DRAW → VERIFY: all passed
...
## Known Issues
- VRF proof posting to Aleph likely failing silently — draws succeed on-chain but `postToAleph()` doesn't post. Aleph API returns empty for Raffle #4.
...
- Theme: Animal Crossing × Chinese Lunar New Year Fire Horse 🐴🔥
...
- **Not yet deployed to mainnet** — user needs to create Safe multisig on Celo for treasury first

## BNB Lucky Draw Progress (Feb 13)
- Reskin complete: 12 files updated with Fire Horse CNY theme
- GitHub: https://github.com/clementfrmd/bnblottery
- Vercel project: `prj_BnVtT3BRS0rWTxD8gBzjF7tydmeF`
- Vercel URL: https://bnblottery.vercel.app (deploying, had yarn.lock issue — copied from celottery)
...
- Admin panel at `/admin` — has menu, wines, content management
...
- Auto-draw endpoint returning `Unauthorized` — no auth token/secret found in env or memory
